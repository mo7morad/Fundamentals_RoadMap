...
public class Tree<T>
{
    ...
    private IEnumerable<T> PreOrderTraversal(TreeNode? node)
    {
        ...
        foreach (var value in PreOrderTraversal(node.LeftChild))
            yield return value;
        foreach (var value in PreOrderTraversal(node.RightChild))
            yield return value;
    }
...
class Program
{
    static void Main(string[] args)
    {
        ...
        personTree.AddLeftChild(new Person("Grandparent", 70), new Person("Parent1", 40));
        personTree.AddRightChild(new Person("Grandparent", 70), new Person("Parent2", 38));
        personTree.AddLeftChild(new Person("Parent1", 40), new Person("Child1", 15));
